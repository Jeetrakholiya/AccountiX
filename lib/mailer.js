...
async function sendOtpEmail(toEmail, otp) {
  ...
  if (supabase) {
    try {
      const { data, error } = await supabase.auth.signInWithOtp({
        email: toEmail,
        options: {
          data: { otp_code: otp, app: 'AccountiX' }
        }
      });
      if (!error) {
  ...
  if (transporter) {
    try {
      ...
      return { provider: 'gmail_smtp', devMode: false, sent: true };
    } catch (mErr) {
      console.warn('Nodemailer SMTP error:', mErr.message);
    }
  }

  // 3. Fallback dev mode
  console.log(`\n======================================================`);
  console.log(`✉️  [DEV MODE / FALLBACK] Gmail & Supabase credentials not set`);
...
async function sendStaffInviteEmail({ toEmail, staffName, role, temporaryPassword, agencyName = 'AccountiX Media HQ', loginUrl = 'https://accountix-phi.vercel.app' }) {
  ...
  const plainText = `Hi ${staffName},\n\nYou have been invited to join ${agencyName} on AccountiX as ${role}.\n\nPortal Login URL: ${loginUrl}\nEmail: ${toEmail}\nTemporary Password: ${temporaryPassword}\n\nPlease use this temporary password to log in for the first time. You will be prompted to create your new personal password upon initial sign-in.\n\nWarm regards,\n${agencyName}`;
  ...
  if (supabase) {
    try {
      if (supabaseServiceRole && supabase.auth && supabase.auth.admin) {
        ...
        if (!invErr) {
          console.log(`✉️ [SUPABASE ADMIN INVITE] Successfully sent invite to ${toEmail}`);
          return { provider: 'supabase_admin', sent: true, devMode: false, gmailComposeUrl, mailtoUrl, plainText };
        } else {
      ...
      if (!otpErr) {
        ...
        return { provider: 'supabase_auth', sent: true, devMode: false, gmailComposeUrl, mailtoUrl, plainText };
      } else {
  ...
  if (transporter) {
    try {
      ...
      await transporter.sendMail(mailOptions);
      return { provider: 'gmail_smtp', sent: true, devMode: false, gmailComposeUrl, mailtoUrl, plainText };
    } catch (mErr) {
